...
public class DaoCidade {
    ...
    public static boolean inserir(Cidade objeto) {
        String sql = "INSERT INTO cidade (nome, sigla, nrhabitantes, dataemancipacao, area, distanciacap) VALUES (?, ?, ?, ?, ?, ?)";
        try {
            ...
            ps.setDouble(6, objeto.getDistanciacap());
     ...
     public static boolean alterar(Cidade objeto) {
        String sql = "UPDATE cidade SET nome = ?, sigla = ?, nrhabitantes = ?, dataemancipacao = ?, area = ?, distanciacap = ? WHERE codigo=?";
        try {
            PreparedStatement ps = conexao.Conexao.getConexao().prepareStatement(sql);
            ps.setString(1, objeto.getNome()); 
            ps.setString(2, objeto.getSigla());
     ...
     public static List<Cidade> consultar() {
        ...
        String sql = "SELECT codigo, nome, sigla, nrhabitantes, dataemancipacao, area, distanciacap FROM cidade";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                objeto.setDistanciacap(rs.getDouble("distanciacap"));
     ...
     public static Cidade consultar(int primaryKey) {
        //editar o SQL conforme a entidade
        String sql = "SELECT codigo, nome, sigla, nrhabitantes, dataemancipacao, area, distanciacap FROM cidade WHERE codigo=?";
        PreparedStatement ps;
        try {
            ps = conexao.Conexao.getConexao().prepareStatement(sql);
